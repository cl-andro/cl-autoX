...
![Maven Central](https://img.shields.io/maven-central/v/com.clauto.cl.insets/insets)
...
```groovy
dependencies {
    implementation "com.clauto.cl.insets:insets:<version>"
}
```
...
If you are already using `MaterialActivity` from `com.clauto.cl.material:material`, nothing needs to be done.
...
This library requires custom attributes for all views, including framework views, a special `LayoutInflater.Factory2` is required. We have a library `com.clauto.cl.layoutinflater:layoutinflater` ([learn more](https://github.com/ClautoW/ClautoX/tree/master/layoutinflater)).
...
* Add the missing "s" to the attribute `fitsSystemWindowsInsets` and `layout_fitsSystemWindowsInsets`
  ...
  If you are also using `com.clauto.cl.material:material-chooser`, since it depends on the library, you should upgrade to `1.4.0` or above.
